...
//};
//char* CLASSES[4] = { "__background__",
//"bike", "vehicle", "pedestrian"
//};
...
DEFINE_string(out_video, "",
    "If provided, store the detection results in the out_video.");
...
int main(int argc, char** argv) {
  ...
  const string& out_video = FLAGS_out_video;
  ...
  const string &indir = "//data_test";
  std::string file;
  out << file_type <<"demo";
  if (file_type == "image")
  {
	  char buf[1000];
	  sprintf(buf, "%s/*.jpg", "data_test//");
	  ...
	  for (size_t k = 0; k < fn.size(); ++k)
	  {
		  ...
		  CHECK(!img.empty()) << "Unable to decode image " << fn[k];
		  Timer batch_timer;
		  batch_timer.Start();
	  ...
	  for (size_t k = 0; k < fn.size(); ++k)
	  {
		  ...
		  while (true) {
			  ...
			  if (count <= max)
			  {
				  ...
				  if (count == 0) {
					  writer.open(out_video, cv::VideoWriter::fourcc('D', 'I', 'V', 'X'), 30, size);
				  }
				  else if (count == max) {
					  writer << img;
					  writer.release();
