...
DeconvolutionIVSLayer<Dtype>::DeconvolutionIVSLayer(
...
template <typename Dtype>
void DeconvolutionIVSLayer<Dtype>::LayerSetUp(
  ...
  vector<int> IVS_weights_trans_shape(2);
  IVS_weights_trans_shape[0] = this->conv_out_channels_;
  IVS_weights_trans_shape[1] = this->conv_in_channels_ / this->group_;
  for (int i = 0; i < this->num_spatial_axes_; ++i) {
    IVS_weights_trans_shape.push_back(kernel_shape_data[i]);
  }
  this->IVS_weights_trans_.Reshape(IVS_weights_trans_shape);
...
template <typename Dtype>
void DeconvolutionIVSLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  this->IVS_conv_inter_shape_.push_back(this->conv_out_channels_*this->parallel_factor_);
...
#else
  if(this->IVS_analyze_add_max_inter_.count() < this->IVS_conv_inter_.count()){
    if(this->analyze_mode_ == QuantizationParameter_Analyze_mode_ANALYZE_ADD\
        || this->analyze_mode_ == QuantizationParameter_Analyze_mode_ANALYZE_BOTH){
...
template <typename Dtype>
void DeconvolutionIVSLayer<Dtype>::Forward_cpu(
      const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  for (int i = 0; i < bottom.size(); ++i) {
    ...
    for (int n = 0; n < this->num_; ++n) {
      ...
      case QuantizationParameter_Rounding_time_EVERY_OPERATION:
        this->backward_cpu_IVS(bottom_data + n * this->bottom_dim_, weight,
            top_data + n * this->top_dim_);
        break;
...
template <typename Dtype>
void DeconvolutionIVSLayer<Dtype>::backward_cpu_IVS(const Dtype* output,
  ...
  Dtype* output_data;
  output_data = col_buff;
  for (int g = 0; g < this->group_; ++g){
    weights += this->kernel_dim_ * this->conv_out_channels_/this->group_ * g;
    for( int kernels_col = 0; kernels_col < this->kernel_dim_; kernels_col++){
      //element-wise multiplication
      memcpy((Dtype*)this->IVS_conv_inter_.cpu_data(),
            output + this->output_offset_*g, sizeof(Dtype) *IVS_conv_inter_.count());
      IVS_conv_inter_data = this->IVS_conv_inter_.mutable_cpu_data();
      for (int kernels_row = 0;
            kernels_row < this->conv_out_channels_/this->group_;\
            kernels_row++){
        caffe_cpu_axpby(this->conv_out_spatial_dim_, (Dtype)0, (Dtype*)IVS_conv_inter_data, \
                        (Dtype)*(weights+kernels_row*this->kernel_dim_+kernels_col),
                        (Dtype*)IVS_conv_inter_data);
        IVS_conv_inter_data += this->conv_out_spatial_dim_;
        //weights++;
      }
      // Analyze after multiplier
      if(this->analyze_mode_ == QuantizationParameter_Analyze_mode_ANALYZE_MULTIPLY\
        || this->analyze_mode_ == QuantizationParameter_Analyze_mode_ANALYZE_BOTH){
        this->Analyze_multiply(this->IVS_conv_inter_.cpu_data(),\
      ...
      for (int IVS_conv_inter_col_left = this->conv_out_channels_/this->group_;\
        ...
        for (int IVS_conv_inter_col = IVS_conv_inter_col_left/2 ; IVS_conv_inter_col;\
             IVS_conv_inter_col--){
          caffe_add(this->conv_out_spatial_dim_, IVS_conv_inter_data, \
                    IVS_conv_inter_data \
                    + this->conv_out_spatial_dim_ * (int)(IVS_conv_inter_col_left/2),
                    IVS_conv_inter_data);
          IVS_conv_inter_data += this->conv_out_spatial_dim_;
        }
        // Analyze afer adder
        if(this->analyze_mode_ == QuantizationParameter_Analyze_mode_ANALYZE_ADD\
          || this->analyze_mode_ == QuantizationParameter_Analyze_mode_ANALYZE_BOTH){
          this->Analyze_add(this->IVS_conv_inter_.cpu_data(),\
                            (IVS_conv_inter_col_left/2) * this->conv_out_spatial_dim_);
        }
        ...
        if((IVS_conv_inter_col_left)%2 == 1){
          IVS_conv_inter_data = this->IVS_conv_inter_.mutable_cpu_data();
          caffe_add(this->conv_out_spatial_dim_, IVS_conv_inter_data, \
                    IVS_conv_inter_data + this->conv_out_spatial_dim_ * (IVS_conv_inter_col_left-1),
                    IVS_conv_inter_data);
          // Analyze afer adder
          if(this->analyze_mode_ == QuantizationParameter_Analyze_mode_ANALYZE_ADD\
            || this->analyze_mode_ == QuantizationParameter_Analyze_mode_ANALYZE_BOTH){
      ...
      IVS_conv_inter_data = this->IVS_conv_inter_.mutable_cpu_data();
      for (int input_row = 0 ; input_row < this->conv_out_spatial_dim_ ;\
              input_row++){
...
template <typename Dtype>
void DeconvolutionIVSLayer<Dtype>::Backward_cpu(
...
#ifdef CPU_ONLY
STUB_GPU(DeconvolutionIVSLayer);
#endif

INSTANTIATE_CLASS(DeconvolutionIVSLayer);
REGISTER_LAYER_CLASS(DeconvolutionIVS);
