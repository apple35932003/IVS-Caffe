...
void ScaleIVSLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void ScaleIVSLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  const ScaleParameter& param = this->layer_param_.scale_param();
...
template <typename Dtype>
void ScaleIVSLayer<Dtype>::Forward_cpu(
  ...
  //Trim Weights
  if (this->phase_ == TEST){
    this->Trim2FixedPoint_cpu(scale_data,
                    scale_dim_, this->bw_params_, this->rounding_, this->fl_params_);
  }
  ...
  //Trim after multiply
  if (this->phase_ == TEST){
    this->Trim2FixedPoint_cpu(top[0]->mutable_cpu_data(),
                    top[0]->count(), this->bw_multiply_, this->rounding_, this->fl_multiply_);
  }
  ...
  if (bias_layer_) {
    ...
    if (this->phase_ == TEST){
      this->Trim2FixedPoint_cpu(top[0]->mutable_cpu_data(),
                      top[0]->count(), this->bw_add_, this->rounding_, this->fl_add_);
    }
...
template <typename Dtype>
void ScaleIVSLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(ScaleIVSLayer);
#endif

INSTANTIATE_CLASS(ScaleIVSLayer);
REGISTER_LAYER_CLASS(ScaleIVS);
