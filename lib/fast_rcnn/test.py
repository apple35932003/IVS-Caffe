...
def im_detect(net, im, _t, boxes=None):
    ...
    if cfg.TEST.SVM:
        ...
        if 'cls_prob' in blobs_out:
            scores = blobs_out['cls_prob']
        #print scores
        #print("------------------")
        #print blobs_out['cls_prob2']
        #print("------------------")
        #scores=scores+blobs_out['cls_prob2']
        #print scores
    ...
    if 'rpn_scores' in blobs_out and 'rpn_boxes' in blobs_out:
        return blobs_out['rpn_scores'] ,blobs_out['rpn_boxes']
    else:
        return scores, pred_boxes
 
    #if 'rpn_scores' in blobs_out and 'rpn_boxes' in blobs_out and 'rpn_cls_scores' in blobs_out :
    #    return blobs_out['rpn_scores'] ,blobs_out['rpn_boxes']/im_scales[0][0],blobs_out['rpn_cls_scores']
    #elif 'rpn_scores' in blobs_out and 'rpn_boxes' in blobs_out:
    #    return blobs_out['rpn_scores'] ,blobs_out['rpn_boxes']/im_scales[0][0]
    #else:
    #    return scores, pred_boxes
...
def test_net_silent(net, imdb, max_per_image=100, thresh=0.05, vis=False):
    """Test a Fast R-CNN network on an image database."""
    num_images = len(imdb.image_index)
    # all detections are collected into:
    #    all_boxes[cls][image] = N x 5 array of detections in
    ...
    #print 'Evaluating detections'
    imdb.evaluate_detections_silent(all_boxes, output_dir)
    return np.mean(imdb._aps)
