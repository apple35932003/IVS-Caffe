...
def quantize_net_adder(net, net_proto, net_adder_IL, adder_bw):
    for layer in net_proto.layer:
        if layer.type == 'ConvolutionIVS':
            adder_IL = net_adder_IL[layer.name] 
...
def quantize_net_multiplier(net, net_proto, net_multiplier_IL, multiplier_bw):
    for layer in net_proto.layer:
        if layer.type == 'ConvolutionIVS':
            multiplier_IL = net_multiplier_IL[layer.name] 
...
if __name__ == '__main__':
    ...
    convert_net_to_qnet_BAC(args.prototxt, args.prototxt_quantized_BAC)
    ...
    net_adder_IL, net_multiplier_IL = analyze_net_adder_multiplier_IL(net_BAC, net_proto_BAC, imdb, max_per_image=args.max_per_image, vis=args.vis)
    ...
    quantize_net_adder(net_BAC, net_proto_BAC, net_adder_IL, bw_adder)
    quantize_net_multiplier(net_BAC, net_proto_BAC, net_multiplier_IL, bw_multiplier)
    write_to_prototxt(net_proto_BAC, './temp_f.prototxt')
