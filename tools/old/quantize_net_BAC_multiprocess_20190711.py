...
if __name__ == '__main__':
    ...
    for i in range(bw_output, bw_output+3):
        net_proto_BAC = read_from_prototxt(args.prototxt_quantized_BAC)
        ...
        jobs.append(p)
        p.start()
    for i in range(bw_output+3, bw_output+6):
        net_proto_BAC = read_from_prototxt(args.prototxt_quantized_BAC)
        quantize_net_multiplier(net_proto_BAC, net_multiplier_IL, i, 0)
        write_to_prototxt(net_proto_BAC, './temp'+str(i)+'.prototxt')
        p = multiprocessing.Process(target=mAP_worker, args=('32-32-32-32-'+str(i), 
    ...
    for proc in jobs:
        proc.join()
    for i in range(bw_output, bw_output+6):
        print '\t{}bit:\t\t{}'.format(i,shared_dict['32-32-32-32-'+str(i)])
    for i in range(bw_output, bw_output+6):
        if shared_dict['32-32-32-32-'+str(i)] > (layer_ap - 0.005):
            bw_multiplier = i
    ...
    for i in range(bw_output, bw_output+6):
        if shared_dict['32-32-32-'+str(i)+'-32'] > (layer_ap - 0.005):
            bw_adder = i
            break;
    #bw_h = 16
    #bw_l = 0
