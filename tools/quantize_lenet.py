...
def analyze_net_output_IL(net, net_proto, max_per_image=100, thresh=0.05, vis=False):
    net_output_IL = dict()
    net_input_IL = dict()
    for layer in net_proto.layer:
        if layer.type == 'ConvolutionIVS' \
...
def test_qnet(net_path, caffemodel_path):
    net = caffe.Net(net_path, caffemodel_path, caffe.TEST)
    score = 0.0
    for i in range(100):
        #net.forward()
        #test = net.forward()['accuracy']
        #print test
        #print str(i) +' ' + str(net.blobs['accuracy'].data[...])
        #print str(i)+' '+str(net.blobs['loss'].data[...])
        #sys.exit()
        score += net.forward()['accuracy']
    return score/100.0
...
def mAP_worker(i, net_path, shared_dict, GPU_ID):
    ...
    ap = test_qnet(net_path, args.caffemodel)
    shared_dict[i] = ap
...
def analyze_net_output_IL_worker(net_output_IL, net_input_IL):
    ...
    net_output_IL_, net_input_IL_ = analyze_net_output_IL(net, net_proto, max_per_image=args.max_per_image, vis=args.vis)
...
if __name__ == '__main__':
    ...
    if has_conv:
        ...
        while not_found:
            ...
            p1.start()
            net_proto = read_from_prototxt(args.prototxt_quantized)
            quantize_net_conv(net_proto, net_param_IL,  i+1, 0)
            quantize_net_deconv(net_proto, net_param_IL, i+1, 0)
    ...
    if has_fc:
        ...
        while not_found:
            ...
            jobs = []
            net_proto = read_from_prototxt(args.prototxt_quantized)
            quantize_net_fc(net_proto, net_param_IL,  i, -1)
            write_to_prototxt(net_proto, './temp'+str(i)+'.prototxt')
            ...
            p1.start()
            net_proto = read_from_prototxt(args.prototxt_quantized)
            quantize_net_fc(net_proto, net_param_IL,  i+1, -1)
            write_to_prototxt(net_proto, './temp'+str(i+1)+'.prototxt')
    ...
    while not_found:
        ...
        jobs = []
        net_proto = read_from_prototxt(args.prototxt_quantized)
        quantize_net_output(net_proto, net_output_IL, net_input_IL, i, -1)
        write_to_prototxt(net_proto, './temp'+str(i)+'.prototxt')
        ...
        p1.start()
        net_proto = read_from_prototxt(args.prototxt_quantized)
        quantize_net_output(net_proto, net_output_IL, net_input_IL, i+1, -1)
        write_to_prototxt(net_proto, './temp'+str(i+1)+'.prototxt')
    ...
    quantize_net_fc(net_proto, net_param_IL, bw_fc, -1)
    quantize_net_output(net_proto, net_output_IL, net_input_IL, bw_output, -1)
    ...
    while not_found:
        ...
        p1.start()
        net_proto_BAC = read_from_prototxt(args.prototxt_quantized_BAC)
        quantize_net_multiplier(net_proto_BAC, net_multiplier_IL, i+1, 0)
    ...
    while not_found:
        ...
        p1.start()
        net_proto_BAC = read_from_prototxt(args.prototxt_quantized_BAC)
        quantize_net_adder(net_proto_BAC, net_adder_IL, i+1, 0)
        write_to_prototxt(net_proto_BAC, './temp'+str(i+1)+'.prototxt')
    ...
    quantize_net_fc(net_proto_final, net_param_IL, bw_fc, -1)
    quantize_net_output(net_proto_final, net_output_IL, net_input_IL, bw_output, -1)
