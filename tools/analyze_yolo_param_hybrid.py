...
def change_layer_bw(net_proto, layer_name, 
    ...
    for l in net_proto.layer:
        if l.name == layer_name:
            ...
            if bw_bias is not None:
                l.quantization_param.bw_bias = int(bw_bias)
            ...
            if fl_bias is not None:
                l.quantization_param.fl_bias = int(fl_bias)
    return net_proto
...
def quantize_net_output(net_proto, net_output_IL, net_input_IL, output_bw, extra_IL):
    ...
    for layer in net_proto.layer:
        if layer.type == 'ConvolutionIVS' \
            ...
            #input_FL = output_FL
def quantize_net_output_layer(layer_name, net_proto, net_output_IL, net_input_IL, output_bw, extra_IL):
    input_bw = output_bw;
    #input_FL = 0;
    for layer in net_proto.layer:
        if layer.type == 'ConvolutionIVS' \
...
def quantize_net_conv(net_proto, net_param_IL, net_bias_IL, weighting_bw, bias_bw, extra_IL):
    for layer in net_proto.layer:
        if layer.type == 'ConvolutionIVS':
            weighting_IL = net_param_IL[layer.name] + extra_IL
            weighting_FL = weighting_bw - weighting_IL
            bias_IL = net_bias_IL[layer.name] + extra_IL
            bias_FL = bias_bw - bias_IL
            change_layer_bw(net_proto, layer.name, \
                            layer.quantization_param.bw_layer_in, \
                            layer.quantization_param.fl_layer_in, \
                            layer.quantization_param.bw_layer_out, \
                            ...
                            layer.quantization_param.bw_multiply, \
                            layer.quantization_param.fl_multiply, \
                            bw_bias = bias_bw, fl_bias = bias_FL, \
                            )
def quantize_net_conv_layer(layer_name ,net_proto, net_param_IL, net_bias_IL, weighting_bw, bias_bw, extra_IL):
    for layer in net_proto.layer:
        if layer.type == 'ConvolutionIVS' and layer.name == layer_name:
            weighting_IL = net_param_IL[layer.name] + extra_IL
            weighting_FL = weighting_bw - weighting_IL
            bias_IL = net_bias_IL[layer.name] + extra_IL
            bias_FL = bias_bw - bias_IL
...
def analyze_net_param_IL_worker(net_param_IL, net_bias_IL, GPU_ID):
    cfg.GPU_ID = GPU_ID
    ...
    net_proto = read_from_prototxt(args.prototxt_quantized)
    net = caffe.Net(args.prototxt_quantized, args.caffemodel, caffe.TEST)
    net_param_IL_, net_bias_IL_ = analyze_net_param_IL(net, net_proto)
...
if __name__ == '__main__':
    ...
    bw_adder = 16 #just initial
    bw_multiplier = 16 #just initial
    bw_bias = 8
    convIL_reduction = 0
    ...
    net_bias_IL = manager.dict()
    p = multiprocessing.Process(target=analyze_net_param_IL_worker,
                                args=(net_param_IL, net_bias_IL, GPU1, ))
    p.start()
    p.join()

    param_bw = manager.dict()

    if has_conv:
        print 'Analyzing CONV and DECONV'
        print '\tbit width\t accuracy'
        net_proto_ori = read_from_prototxt(args.prototxt_quantized)
        for layer in net_proto_ori.layer:
            if layer.type == 'ConvolutionIVS':
                print layer.name
                i = 3
                not_found = True
                while not_found:
                    ...
                    jobs = []
                    net_proto = read_from_prototxt(args.prototxt_quantized)
                    quantize_net_conv_layer(layer.name, net_proto, net_param_IL, net_bias_IL,  i, i, convIL_reduction)
                    ...
                    p1.start()
                    net_proto = read_from_prototxt(args.prototxt_quantized)
                    quantize_net_conv_layer(layer.name, net_proto, net_param_IL, net_bias_IL,  i+1, i+1, convIL_reduction)
                    ...
                    for j in range(i, i+2):
                        if shared_dict[str(j)+'-32-32-32-32'] > (full_ap - args.error_margin):
                            bw_conv = j
                            not_found = False
                            print layer.name, bw_conv
                            param_bw[layer.name]=bw_conv
                            break;
                    i = i + 2
    print param_bw
    ...
    for layer in net_proto_ori.layer:
        if layer.type == 'ConvolutionIVS':
            ...
            while not_found:
                ...
                jobs = []
                net_proto = read_from_prototxt(args.prototxt_quantized)
                quantize_net_output_layer(layer.name ,net_proto, net_output_IL, net_input_IL, i, actIL_reduction)
                write_to_prototxt(net_proto, './temp'+str(i)+'.prototxt')
                ...
                p1.start()
                net_proto = read_from_prototxt(args.prototxt_quantized)
                quantize_net_output_layer(layer.name, net_proto, net_output_IL, net_input_IL, i+1, actIL_reduction)
                write_to_prototxt(net_proto, './temp'+str(i+1)+'.prototxt')
    ...
    for layer in net_proto_ori.layer:
        if layer.type == 'ConvolutionIVS':
            quantize_net_conv_layer(param_bw[layer.name], net_proto, net_param_IL, net_bias_IL, bw_conv, bw_bias, convIL_reduction)
            quantize_net_output_layer(act_bw[layer.name], net_proto, net_output_IL, net_input_IL, bw_output, actIL_reduction)
    #quantize_net_conv(net_proto, net_param_IL, net_bias_IL, bw_conv, bw_bias, convIL_reduction)
    ...
    quantize_net_conv(net_proto_final, net_param_IL, net_bias_IL, bw_conv, bw_bias, convIL_reduction)
