...
def quantize_net_adder(net, net_proto, net_adder_IL, adder_bw, extra_IL):
    for layer in net_proto.layer:
        if layer.type == 'ConvolutionIVS':
...
def quantize_net_multiplier(net, net_proto, net_multiplier_IL, multiplier_bw, extra_IL):
    for layer in net_proto.layer:
        if layer.type == 'ConvolutionIVS':
...
def quantize_net_output(net, net_proto, net_output_IL, net_input_IL, output_bw, extra_IL):
    input_bw = output_bw;
    #input_FL = 0;
    for layer in net_proto.layer:
        if layer.type == 'ConvolutionIVS' \
...
def quantize_net_conv(net, net_proto, net_param_IL, weighting_bw, extra_IL):
    for layer in net_proto.layer:
        if layer.type == 'ConvolutionIVS':
...
def quantize_net_fc(net, net_proto, net_param_IL, weighting_bw, extra_IL):
    for layer in net_proto.layer:
        if layer.type == 'FcIVS':
            weighting_IL = net_param_IL[layer.name] + extra_IL
            weighting_FL = weighting_bw - weighting_IL
...
def quantize_net_deconv(net, net_proto, net_param_IL, weighting_bw, extra_IL):
    for layer in net_proto.layer:
...
if __name__ == '__main__':
    ...
    if has_conv:
        ...
        for i in range(1, 13):
            net_proto = read_from_prototxt(args.prototxt_quantized)
            quantize_net_conv(net, net_proto, net_param_IL,  i, -1)
            quantize_net_deconv(net, net_proto, net_param_IL, i, -1)
    ...
    if has_fc:
        ...
        for i in range(1, 13):
            net_proto = read_from_prototxt(args.prototxt_quantized)
            quantize_net_fc(net, net_proto, net_param_IL,  i, -1)
            write_to_prototxt(net_proto, './temp'+str(i)+'.prototxt')
    ...
    for i in range(1, 13):
        net_proto = read_from_prototxt(args.prototxt_quantized)
        quantize_net_output(net, net_proto, net_output_IL, net_input_IL, i, 0)
        write_to_prototxt(net_proto, './temp'+str(i)+'.prototxt')
    ...
    quantize_net_deconv(net, net_proto, net_param_IL, bw_conv, -1)
    quantize_net_fc(net, net_proto, net_param_IL, bw_fc, -1)
    quantize_net_output(net, net_proto, net_output_IL, net_input_IL, bw_output, 0)
    write_to_prototxt(net_proto, './temp.prototxt')
    ...
    for i in range(bw_output, bw_output+12):
        net_proto_BAC = read_from_prototxt(args.prototxt_quantized_BAC)
        quantize_net_multiplier(net_BAC, net_proto_BAC, net_multiplier_IL, i, 0)
    ...
    for i in range(bw_output, bw_output+12):
        if shared_dict['32-32-32-32-'+str(i)] > (full_ap - args.error_margin):
            bw_multiplier = i
            ap_after_multiplier = shared_dict['32-32-32-32-'+str(i)]
            break;
    net_proto_BAC = read_from_prototxt(args.prototxt_quantized_BAC)
    quantize_net_multiplier(net_BAC, net_proto_BAC, net_multiplier_IL, bw_multiplier, 0)
    ...
    for i in range(bw_output, bw_output+12):
        net_proto_BAC = read_from_prototxt(args.prototxt_quantized_BAC)
        quantize_net_adder(net_BAC, net_proto_BAC, net_adder_IL, i, 0)
        write_to_prototxt(net_proto_BAC, './temp'+str(i)+'.prototxt')
    ...
    for i in range(bw_output, bw_output+12):
        if shared_dict['32-32-32-'+str(i)+'-32'] > (full_ap - args.error_margin):
            bw_adder = i
            break;
    #bw_h = 16
    #bw_l = 0
    ...
    quantize_net_conv(net_final, net_proto_final, net_param_IL, bw_conv, -1)
    quantize_net_deconv(net_final, net_proto_final, net_param_IL, bw_conv, -1)
    quantize_net_fc(net_final, net_proto_final, net_param_IL, bw_fc, -1)
    quantize_net_output(net_final, net_proto_final, net_output_IL, net_input_IL, bw_output, 0)
    quantize_net_multiplier(net_final, net_proto_final, net_multiplier_IL, bw_multiplier, 0)
    quantize_net_adder(net_final, net_proto_final, net_adder_IL, bw_adder, 0)
