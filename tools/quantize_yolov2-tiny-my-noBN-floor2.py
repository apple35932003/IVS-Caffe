...
def convert_net_to_qnet(ori_net_path, q_net_path):
    ...
    for l in net_proto.layer:
        ...
        if l.type == 'Convolution':
            ...
            l.quantization_param.rounding_scheme = 2 #FLOOR
...
def calc_layer_param_IL(net,layer):
    ...
    max_bias = max_bias
    if max_bias == 0:
        return math.ceil(math.log(abs(max_param), 2)) + 1, 1
    else:
...
def calc_layer_inout_IL(net, layer_bottom_name):
    ...
    #output_sorted = np.sort(layer_output, axis = None)
    #layer_output_max = abs(max(output_sorted[int(len(output_sorted)*percentile)], output_sorted[-1*int(len(output_sorted)*percentile)],key=abs))
...
def analyze_net_output_IL(net, net_proto):
    ...
    for layer in net_proto.layer:
        ...
        if layer.type == 'ConvolutionIVS' \
            ...
            net_output_IL[layer.name] = 0
            net_input_IL[layer.name] = 0
    for i in tqdm.tqdm(xrange(num_iters)):
        print i
        #if cfg.TEST.HAS_RPN:
        #    box_proposals = None
        #else:
        ...
        for layer in net_proto.layer:
            if layer.type == 'ConvolutionIVS' \
                or layer.type == 'FcIVS' \
                or layer.type == 'DeconvolutionRistretto':
                net_output_IL[layer.name] += calc_layer_inout_IL(net, layer.top[0])
                net_input_IL[layer.name] += calc_layer_inout_IL(net, layer.bottom[0])
    ...
    for layer in net_proto.layer:
        if layer.type == 'ConvolutionIVS' \
            or layer.type == 'FcIVS' \
            or layer.type == 'DeconvolutionRistretto':
            net_output_IL[layer.name] = net_output_IL[layer.name]/num_iters
            net_input_IL[layer.name] = net_input_IL[layer.name]/num_iters
