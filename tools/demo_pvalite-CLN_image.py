...
CLASSES = ('__background__', 'pedestrian','vehicle', 'bike')
...
def vis_detections(im, class_name, dets, cls, frame_id,res_out=None,thresh=0.5):
    ...
    for i in inds:
        ...
        if cls == 'pedestrian':
            ...
            if(args.dis==0):
                if(args.pr==1):
                    cv2.putText(im,str(cls),(bbox[0],bbox[1]), cv2.FONT_HERSHEY_DUPLEX ,0.6,(255,116,21))
            else:
            ...
            if(args.dis==0):
                if(args.pr==1):
                    cv2.putText(im,str(cls),(bbox[0],bbox[1]), cv2.FONT_HERSHEY_DUPLEX ,0.6,(0,255,0))
            else:
                if bbox[3] != global_h_position:
                    distance = camera_height * camera_focal / float(bbox[3] - global_h_position)
                    distance_str = str(int(np.floor(distance)))
            ...
            if(args.dis==0):
                if(args.pr==1):
                    cv2.putText(im,str(cls),(bbox[0],bbox[1]), cv2.FONT_HERSHEY_DUPLEX ,0.6,(0,0,255))
            else:
                if bbox[3] != global_h_position:
                    distance = camera_height * camera_focal / float(bbox[3] - global_h_position)
                    distance_str = str(int(np.floor(distance)))
...
def demo(net, image_name, task, iteration, dataset):
    ...
    print out_path+'/'+video_name+'_'+out_name+'_'+iteration+'_'+dataset+'_'+str(CONF_THRESH)+'.png'
    cv2.imwrite(os.path.join(out_path,video_name+'_'+out_name+'_'+iteration+'_'+dataset+'_'+str(CONF_THRESH)+'.png'), frame)
    #out.write(frame)
    #cv2.imshow('frame',frame)
...
def parse_args():
    ...
    parser.add_argument('--image', dest='image', help='image_path',
...
if __name__ == '__main__':
    ...
    demo(net, image, task, iteration, dataset)
