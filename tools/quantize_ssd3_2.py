...
if __name__ == '__main__':
    ...
    bw_deconv = 8 #just initial
    bw_fc = 8 #just initial
    ...
    net_proto = read_from_prototxt(args.prototxt_quantized)
    write_to_prototxt(net_proto, './2temp.prototxt')
    p = multiprocessing.Process(target=mAP_worker, args=('DQ-DQ-DQ-32-32', './2temp.prototxt',
                                 shared_dict, GPU1))
    p.start()
    p.join()
    ...
    if args.accumulator_analysis == None:
        ...
        p.start()
        p.join()
        with open('2accumulator_analysis.json', 'w') as outfile:
    ...
    write_to_prototxt(net_proto_final, './2temp_f.prototxt')
    p = multiprocessing.Process(target=mAP_worker, args=('DQ-DQ-DQ-DQ-DQ', './2temp_f.prototxt',
                                 shared_dict,GPU1))
    p.start()
    p.join()
