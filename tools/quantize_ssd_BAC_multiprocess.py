...
def mAP_worker(i, net_path, shared_dict):
    ...
    ap = float(check_output('./caffe-fast-rcnn/build/tools/caffe test_detection --model=' + net_path + ' --weights=' + args.caffemodel + ' -iterations='+str(num_images),shell=True))
...
if __name__ == '__main__':
    ...
    p = multiprocessing.Process(target=analyze_net_param_IL_worker, args=(net_param_IL,))
    p.start()
    p.join()
    ...
    if has_conv:
        ...
        for i in range(1, 13):
            net_proto = read_from_prototxt(args.prototxt_quantized)
            quantize_net_conv(net_proto, net_param_IL,  i, -1)
            quantize_net_deconv(net_proto, net_param_IL, i, -1)
    ...
    net_proto = read_from_prototxt(args.prototxt_quantized)
    quantize_net_conv(net_proto, net_param_IL, bw_conv, -1)
    quantize_net_deconv(net_proto, net_param_IL, bw_conv, -1)
    ...
    quantize_net_conv(net_proto_final, net_param_IL, bw_conv, -1)
    quantize_net_deconv(net_proto_final, net_param_IL, bw_conv, -1)
